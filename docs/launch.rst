...
.. code-block:: python
    ...
    def on_mouse_move(boss: Boss, window: Window, data: dict[str, Any]) -> None:
        ...
        # contain x and y (the cell column/row under the mouse) and the
        # modifier keys ctrl, shift, alt and super.
        ...
...
kitty rate-limits ``on_mouse_move`` dispatch to once every 50ms, but does no
further throttling, debouncing or caching, so do your own caching too if
scanning the whole viewport on every call is still too expensive for your
pattern. Pass ``url=None`` to ``screen.set_hyperlink_for_range()`` to remove
a hyperlink from a range of cells instead of setting one.
